...
class Translator (object) :
    ...
    def do_testlist_comp (self, st, ctx=ast.Load) :
        ...
        """
        if st[1].text == "," :
            return None, [self.do(child, ctx) for child in st[::2]]
        else :
            return self.do(st[0], ctx), self.do(st[1], ctx)[0]
    ...
    def do_dictorsetmaker (self, st, ctx=ast.Load) :
        ...
        if st[1].text == ":" :
            ...
            loop, elts = self.do_testlist_comp(st, ctx)
            if loop is None :
